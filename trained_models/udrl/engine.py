...
lr_decayer = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, threshold=0.005, min_lr=0.00001, patience=patience)
l1_loss = nn.L1Loss()
contrast_loss = nn.CrossEntropyLoss()
loss = {'l1': l1_loss, 'contrast': contrast_loss}

## Execute Training
train_model(model, dataloaders, loss, optimizer, lr_decayer, num_epochs=epochs)
